...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        // Insert form details
        $stmt = $conn->prepare("INSERT INTO forms_combined (title, description, form_type, created_by, created_for, firstname, lastname, email, number) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
EOD;
        ...
        $stmt = $conn->prepare("UPDATE forms_combined SET questions_json = ? WHERE id = ?");
        $stmt->execute([$questionsJson, $form_id]);
