...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $questions_json = isset($_POST['questions_json']) ?1:0;
    ...
    if ($created_for === null || $created_for === '') {
       ...
       if (!empty($company_name)) {
        ...
        $stmt = $conn->prepare("SELECT id FROM demo_requests WHERE email = ? LIMIT 1");
        ...
        if ($existingUser) {
            ...
            $stmt = $conn->prepare("INSERT INTO demo_requests (email, password, business_name, role_id, email) VALUES (?, '123456', ?, ?, ?)");
        ...
        if (isset($_FILES['company_logo']) && $_FILES['company_logo']['error'] === UPLOAD_ERR_OK) {
            ...
            if (move_uploaded_file($_FILES['company_logo']['tmp_name'], $upload_dir . $safe_name)) {
                ...
                $update_stmt = $conn->prepare("UPDATE demo_requests SET profile_image = ? WHERE id = ?");
    ...
    if ($created_for !== null && $created_for !== '' && empty($company_name)) {
        ...
        $stmt = $conn->prepare("SELECT business_name FROM demo_requests WHERE id = ?");
    ...
    try {
        ...
        // Insert form details
        $stmt = $conn->prepare("INSERT INTO demo_forms_combined (title, description, form_type, created_by, created_for, firstname, lastname, email, number, questions_json, thankyou_message, allow_another_response) VALUES (?, ?, ?, ?, ?,?, ?, ?, ?, ?, ?, ?)");
...
EOD;
        ...
        // var_dump($questionsArr); // Debugging line to check options
                        // exit;
         $questionsJson = json_encode($questionsArr, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);

    $stmt = $conn->prepare("UPDATE demo_forms_combined SET questions_json = :questions_json, thankyou_message = :thankyou_message WHERE id = :id");
    $stmt->bindParam(':questions_json', $questionsJson, PDO::PARAM_STR);
    $stmt->bindParam(':thankyou_message', $thankyou_message, PDO::PARAM_STR);
