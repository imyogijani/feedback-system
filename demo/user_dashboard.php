...
$isUserLogin = isset($_SESSION['role_id']) && $_SESSION['role_id'] == 4;
...
try {
    // Count total forms for current user
    $stmt = $conn->prepare("SELECT COUNT(*) AS total FROM demo_forms_combined WHERE created_by = :user_id");
    ...
    // Correct user-specific form fetch with creator details
    $stmt = $conn->prepare("
        SELECT 
            f.id, 
            f.title, 
        ...
        FROM demo_forms_combined f
        LEFT JOIN demo_requests u ON f.created_by = u.id
        WHERE (f.created_by = :user_id OR f.created_for = :user_id) AND u.role_id = 4
