...
try {
    ...
    $stmtResponse = $conn->prepare("INSERT INTO form_responses_combined (form_id, firstname, lastname, email, number, responses_json, submitted_at) VALUES (?, ?, ?, ?, ?, ?, NOW())");
    ...
    header("Location: thankyou.php?form_id=" . urlencode($form_id));
    exit;
} catch (Exception $e) {
    ...
    header("Location: thankyou.php?id=" . urlencode($form_id));
    exit;
}
