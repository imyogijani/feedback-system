...
if (isset($conn) && $conn instanceof PDO) {
    ...
    echo "Database connection not established. Please check your configuration.";
    exit();
}
...
// Fetch questions from questions_json field (sectioned format)
$questions = [];
            ...
            foreach ($questions as $section):
                ...
                if (!empty($section['questions']) && is_array($section['questions'])) {
                    foreach ($section['questions'] as $q) {
                        ...
                        $qType = isset($q['type']) ? $q['type'] : (isset($q['question_type']) ? $q['question_type'] : 'text');
                        $opts = isset($q['options']) && is_array($q['options']) ? $q['options'] : [];
                        $qName = 'q_' . $qSerial;
            ?>
                <div class="mb-3">
                    <label class="form-label" style="font-weight: 500;">
                        <?= ($qSerial++) . '. ' . htmlspecialchars($qText) ?>
                    </label>
                    <?php
                    switch ($qType) {
                        case 'text':
                        ...
                        case 'radio':
                            if ($opts) {
                                ...
                                echo '<div class="form-check"><input class="form-check-input" type="radio" name="' . $qName . '"><label class="form-check-label">Option 1</label></div>';
                            }
                            break;
                        case 'checkbox':
                            if ($opts) {
                                ...
                                echo '<div class="form-check"><input class="form-check-input" type="checkbox" name="' . $qName . '[]"><label class="form-check-label">Option 1</label></div>';
                            }
                            break;
                        case 'dropdown':
