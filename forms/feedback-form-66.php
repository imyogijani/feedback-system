...
$stmt = $conn->prepare("SELECT * FROM forms_combined WHERE form_id = ? ORDER BY id ASC");
$stmt->execute([$formId]);
        ...
        <form method="POST" action="process_response.php">
            ...
            <?php foreach ($questions as $q): ?>
                <div class="mb-3">
                    ...
                    $stmt = $conn->prepare("SELECT * FROM forms_combined WHERE question_id = ? ORDER BY id ASC");
                    $stmt->execute([$q['id']]);
